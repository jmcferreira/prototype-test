...
public interface ICardResolver
{
    ...
    /// </summary>
    List<HexCoord> GetValidTargets(CardData data, Unit caster, Unit enemy, HexGrid grid);

    /// <summary>
    /// Execute the effect on the chosen target hex. Only called with
    /// a coord that was in the GetValidTargets list.
    /// </summary>
    void Resolve(CardData data, Unit caster, Unit enemy, HexGrid grid, HexCoord target);
}
